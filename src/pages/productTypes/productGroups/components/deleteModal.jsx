...
function DeleteModal({actionGroup ,deleteModal, setDeleteModal}) {
    ...
    return (
        <div className='G-modal-menu'>
            ...
            <div className="modalCont G-box-shadow">
                ...
                <p className="prg">{t("products.prg1")} <span>({actionGroup.name})</span>?</p>


                <div className="buttonsCont">
                    <button onClick={() => setDeleteModal(false)} className="cancelBtn">{t("products.btn6")}</button>
                    <button onClick={()=> DeleteGroup(actionGroup)} className="deleteBtn">{t("products.btn7")}</button>
                </div>
            </div>
        </div>
    );
